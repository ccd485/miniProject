...
public class LogDialog extends JDialog {
	...
	JLabel indate_lb = new JLabel("입고일", JLabel.RIGHT);
	JTextField indate_tf = new JTextField();
	JPanel indate_p = new JPanel();
	...
	protected void center_setting() {
		...
		center_p.add(quantity_p);
		indate_p.setLayout(new BorderLayout());
		indate_p.add("West", indate_lb);
		indate_p.add(indate_tf);
		center_p.add(indate_p);
	}
	...
	public Log_DTO input() {
		...
		String indate = indate_tf.getText().trim();
		Log_DTO ld = new Log_DTO(item, Integer.parseInt(price), Integer.parseInt(quantity), indate);
		return ld;
	}
